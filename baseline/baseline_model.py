...
class ResNet1D(nn.Module):
    ...
    def __init__(self, num_classes=10, dropout_rate=0.3, use_preprocessing=False):
    ...
    def forward(self, x):
        ...
        # 区間ごとの特徴を抽出（10区間）
        x = self.adaptive_pool(x)  # (batch_size, 512, 10)
        
        # フラット化
        x = x.view(x.size(0), -1)  # (batch_size, 512 * 10)
...
class ImprovedCNN(nn.Module):
    ...
    def __init__(self, num_classes=10, dropout_rate=0.5):
        ...
        # 10区間 × 300ポイント = 3000ポイント
        # より細かく区間を捉えるため、Adaptive Poolingを使用
        self.adaptive_pool = nn.AdaptiveAvgPool1d(10)  # 10区間の特徴を抽出
    ...
    def forward(self, x):
        ...
        # 区間ごとの特徴を抽出（10区間）
        x = self.adaptive_pool(x)  # (batch_size, 256, 10)
        
        # フラット化
        x = x.view(x.size(0), -1)  # (batch_size, 256 * 10)
...
class MultiScaleResNet1D(nn.Module):
    ...
    def __init__(self, num_classes=10, dropout_rate=0.3, use_preprocessing=True):
        ...
        self.fc1 = nn.Linear(total_features * 10, 512)
        self.dropout1 = nn.Dropout(dropout_rate)
        self.fc2 = nn.Linear(512, 256)
        self.dropout2 = nn.Dropout(dropout_rate)
        self.fc3 = nn.Linear(256, num_classes)
    ...
    def _make_small_resnet(self, out_channels):
        ...
        layers.append(nn.AdaptiveAvgPool1d(10))
